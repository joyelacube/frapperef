...
def get_context(context):
    ...
    pro=frappe.get_doc("Project",{"tracking_id":tracking_id})
    context.prodetails=pro
    print(pro)

    caddress=frappe.get_doc("Address",pro.address)
    context.caddress=caddress
    ...
    if frappe.db.exists('Quotation',{'project_id':pro.name}):
        quotation=frappe.get_doc('Quotation',{'project_id':pro.name})
        context.quotation=quotation


    
    protasks=frappe.get_list("Task",filters={"project": pro.name,"show_to_customer":1,"status":"Completed"},order_by='name asc')
    context.protask=protasks
    print(protasks,"33333333333333333333333333333333")
    ncprotasks=frappe.get_list("Task",filters={"project": pro.name,"show_to_customer":1,"status":["not in",["Completed"]]},order_by='name asc')
    ...
    # context.ctrack=ctrack
    # images=[]
    # for i in ctrack.tracking_page_pictures:
    #     images.append(i.images)
    # print(random.sample(images, 4))
    # context.images=random.sample(images, 4)
