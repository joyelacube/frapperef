...
def get_context(context):
    ...
    payment=frappe.get_list("Payment Entry",filters={"project": pro.name,'payment_type':"Receive",'party_type':"Customer","party":pro.customer},order_by='name asc')
    context.payment=payment
    print(payment)
    
    print(payment[-1])

    balance=quotation.rounded_total
    if payment:
        pe=frappe.get_doc("Payment Entry",payment[-1]['name'])
        for i in pe.references:
            balance=i.outstanding_amount-i.allocated_amount
            if balance==0:
                balance="Fully Paid"
