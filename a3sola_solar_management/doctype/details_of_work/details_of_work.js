...
frappe.ui.form.on('Details of Work', {
    ...
    onload: function (frm) {
        ...
        if (prev_route[1] === 'Task') {
            ...
            frappe.call({
                ...
                method: "a3sola_solar_management.a3sola_solar_management.doctype.details_of_work.details_of_work.test",
        ...
        if(r.message.cir!=undefined && r.message.sta!=undefined && r.message.discount!=undefined){
                cur_frm.set_value("circle",r.message.cir);
                cur_frm.set_value("section",r.message.sta);
                cur_frm.set_value("subsidy_amount",r.message.discount);

            }

              cur_frm.set_value("pr",r.message.pr)

				// cur_frm.set_value("contact",r.message.con);

              frm.refresh_field('pr');
                frm.refresh_field('customer_name');
				frm.refresh_field('consumer_number');
                frm.refresh_field('section');
				frm.refresh_field('circle');
        ...
        cur_frm.fields_dict['invoice_no'].get_query = function(doc) {
                ...
                return {
                    filters:{"project":source_doc.project}
